...
import static globals.MyMqtt.publishStringMessage;
...
public class ViewJobActivity extends AppCompatActivity {
    ...
    public static void setTotalPrice() {
        mJobs job = app.db.mJobsDao().get_job(_job_id);
        lbl_total_price.setText(app.ctx.getString(R.string.total_price) + ": " + globals.formatCurrency(job.get_the_total_price()));
        if (job.get_the_total_price() > 0) {
    ...
    public void forward_bill_to_client() {
        ...
        try {
            ...
            json_bill.put("tasks",new Gson().toJson(tasks));
            json_bill.put("_job_id",_job_id);
            json_bill.put("artisan_app_id",artisan.app_id);
            json_bill.put("client_app_id",job.client_app_id);
            json_bill.put("type","client_job_bill_notification");

            //send direct to client
            publishStringMessage(json_bill.toString(),job.client_app_id,0);

            Ion.with(ViewJobActivity.this)
                    .load(globals.base_url + "/forward_bill_to_client")
                    .setBodyParameter("data", json_bill.toString())
                    .asString()
